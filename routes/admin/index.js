...
if(req.session.isLoggedin && req.session.adminId){
    ...
    myForm.label("Username","username");
    myForm.text("username", {class:"form-control"});
    myForm.pass({name:'password', label: "Password", class:'form-control' , validation:[{rule:'isRequired', error: 'This is required.'}] });
...
router.all('/post/add', function(req, res, next) {
